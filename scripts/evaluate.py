...
def main():
    ...
    print(
        f"  QI-EI Pruning: {'Enabled (keep ' + str(int(args.qi_early_keep_ratio * 100)) + '%)' if args.use_qi_early else 'Disabled'}")
    ...
    if args.llm_log_file:
        ...
        if args.use_qi_early:
            log_file.write(
                f"QI-EI: Enabled (keep {args.qi_early_keep_ratio * 100:.0f}%, temp={args.qi_early_temperature})\n")
        log_file.write("=" * 80 + "\n\n")
        log_file.flush()
